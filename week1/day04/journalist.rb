...
# A huge array representing a journalists list
...
def arry_aude(list)
  # handle with "aude" or "Aude"
  counter = 0
  list.each do |item|
    if /aude/i.match(item)
      counter += 1
    end
  end
  return counter
...
def arry_upcase(list)
  # handle start with a capital letter
  counter = 0
  list.each do |item|
    #because a handle start with @, scan the second letter of each
    if item[1] =~ /^[A-Z]/
      counter += 1
    end
  end
...
def arry_capital(list)
  # return number of item in arry with capital letters on them
  counter = 0
  list.each do |item|
    if item =~ /[A-Z]/
      counter += 1
...
def arry_underscore(list)
  # return number of underscores in each items of array given
  i = 0
  list.each do |item|
    i += item.count "_"
  end
...
def display(list)
  ...
  puts "Il y a #{size(list)} journalistes dans cet array."
  separator
  puts "2.Combien d'handle contiennent un numéro ?"
  puts "#{array_nb(list)} contiennent un numero."
  separator
  puts "3.Combien d'handle contiennent les 4 lettres du prénom 'Aude' à la suite (sans faire attention à la casse) ?"
  puts "#{arry_aude(list)} handle contiennent Aude ou aude"
  separator
  puts "4.Combien commencent par une majuscule ?"
  puts "#{arry_upcase(list)} handle commencent par une majuscule"
  separator
  puts "5.Combien contiennent une majuscule ?"
  puts "#{arry_capital(list)} handle contiennent une majuscule"
  separator
  puts "6.Combien de _ dans tous les pseudos confondus ?"
  puts "#{arry_underscore(list)} '_' dans tous les pseudos"
  separator
